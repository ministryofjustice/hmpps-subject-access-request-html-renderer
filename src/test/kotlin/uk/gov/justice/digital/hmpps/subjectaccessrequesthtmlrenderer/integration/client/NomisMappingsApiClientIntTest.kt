...
import uk.gov.justice.digital.hmpps.subjectaccessrequesthtmlrenderer.integration.assertExpectedSubjectAccessRequestException
import uk.gov.justice.digital.hmpps.subjectaccessrequesthtmlrenderer.integration.assertExpectedSubjectAccessRequestExceptionWithCauseNull
import uk.gov.justice.digital.hmpps.subjectaccessrequesthtmlrenderer.integration.client.BaseClientIntTest.Companion.StubErrorResponse
...
class NomisMappingsApiClientIntTest : BaseClientIntTest() {
  ...
  fun setup() {
    ...
    oAuth2AuthorizedClientService.removeAuthorizedClient("sar-html-renderer-client", "anonymousUser")
  }
  ...
  fun `should retry on 5xx error`(stubResponse: StubErrorResponse) {
    ...
    assertExpectedSubjectAccessRequestException(
      actual = exception,
      expectedPrefix = "request failed and max retry attempts (2) exhausted",
  ...
  fun `should not retry on authentication failures`(stubResponse: StubErrorResponse) {
    ...
    assertExpectedSubjectAccessRequestException(
      ...
      expectedCause = stubResponse.expectedException,
      expectedParams = mapOf(
        "cause" to "$AUTH_ERROR_PREFIX ${stubResponse.status.value()} ${stubResponse.status.reasonPhrase} $AUTH_ERROR_SUFFIX: [no body]",
      ),
    )
  }
}
